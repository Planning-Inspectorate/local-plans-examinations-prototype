...
router.post('/projects/back-office/create-case/v5/3-select-LPA', (req, res) => {
  if (!req.body.lpa) {
    ...
    return res.render('projects/back-office/create-case/v5/3-select-LPA', {
      lpaList,
      selectedLPA: '',
      isEdit: req.body.isEdit === 'true',
...
router.post('/projects/back-office/create-case/v5/additional-LPA', (req, res) => {
  if (!req.body.lpa) {
    ...
    return res.render('projects/back-office/create-case/v5/additional-LPA', {
      lpaList,
      selectedLPA: '',
...
const buildContactOptions = (mainContact, contacts = [], lpas = []) => {
  const toLabel = (contact) => {
    ...
    return contact?.fullName || fallbackName || contact?.name || contact?.email || 'Contact';
  };
...
router.post('/projects/back-office/create-case/v5/main-contact', (req, res) => {
  ...
  if (!req.body.mainContactFullName || !req.body.mainContactEmail) {
    return res.render('projects/back-office/create-case/v5/main-contact', {
      lpas,
      mainContact: {
...
router.get('/projects/back-office/create-case/v5/change-main-contact', (req, res) => {
  ...
  res.render('projects/back-office/create-case/v5/change-main-contact', {
...
router.post('/projects/back-office/create-case/v5/change-main-contact', (req, res) => {
  const contacts = req.session.contacts || [];
  ...
  if (!selectedMainContact) {
    return res.render('projects/back-office/create-case/v5/change-main-contact', {
      contactOptions: buildContactOptions(req.session.mainContact, contacts, req.session.lpas || []),
      selectedMainContact: '',
...
router.get('/projects/back-office/create-case/v5/contact-organisation', (req, res) => {
  ...
  if (pending.type === 'main' && req.session.mainContact) {
    ...
    const mainContactName = req.session.mainContact.fullName
      || `${req.session.mainContact.firstName || ''} ${req.session.mainContact.lastName || ''}`.trim()
      || req.session.mainContact.name
      || req.session.mainContact.email
      || 'main contact';
    contactLabel = `main contact (${mainContactName})`;
  } else if (
    pending.type === 'additional' &&
    Number.isInteger(pending.index) &&
    Array.isArray(req.session.contacts) &&
    ...
    const additionalContactName = contact.fullName
      || `${contact.firstName || ''} ${contact.lastName || ''}`.trim()
      || contact.name
      || contact.email
      || 'additional contact';
    contactLabel = `additional contact (${additionalContactName})`;
  }

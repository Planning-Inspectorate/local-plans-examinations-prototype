...
router.get('/projects/back-office/create-case/v4/LPA-region', (req, res) => {
  ...
  res.render('projects/back-office/create-case/v4/LPA-region', {
...
router.post('/projects/back-office/create-case/v4/LPA-region', (req, res) => {
  if (req.body.action === 'cancel') {
    return res.redirect('/projects/back-office/create-case/v4/check-answers');
  }
  ...
  res.redirect('/projects/back-office/create-case/v4/add-additional-lpa');
});
...
router.get('/projects/back-office/create-case/v4/load-case', (req, res) => {
...
// Case officer
router.get('/projects/back-office/create-case/v4/0-case-officer-name', (req, res) => {
  const isEdit = req.query.edit === 'true';
  ...
  res.render('projects/back-office/create-case/v4/0-case-officer-name', {
...
router.post('/projects/back-office/create-case/v4/0-case-officer-name', (req, res) => {
  if (!req.body.caseOfficer) {
    return res.render('projects/back-office/create-case/v4/0-case-officer-name', {
  ...
  if (isEdit) {
    res.redirect('/projects/back-office/create-case/v4/check-answers');
  } else {
    res.redirect('/projects/back-office/create-case/v4/1-plan-title');
  }
});

// Plan title page
router.get('/projects/back-office/create-case/v4/1-plan-title', (req, res) => {
  const isEdit = req.query.edit === 'true';
  res.render('projects/back-office/create-case/v4/1-plan-title', {
    planTitle: req.session.planTitle,
    isEdit: isEdit
  });
});

router.post('/projects/back-office/create-case/v4/1-plan-title', (req, res) => {
  if (!req.body['plan-title']) {
    return res.render('projects/back-office/create-case/v4/1-plan-title', {
  ...
  if (isEdit) {
    ...
    res.redirect('/projects/back-office/create-case/v4/2-plan-type');
  }
});

// Plan type page
router.get('/projects/back-office/create-case/v4/2-plan-type', (req, res) => {
  const isEdit = req.query.edit === 'true';
  res.render('projects/back-office/create-case/v4/2-plan-type', {
    planType: req.session.planType,
    isEdit: isEdit
  });
});

router.post('/projects/back-office/create-case/v4/2-plan-type', (req, res) => {
  if (!req.body['plan-type']) {
    return res.render('projects/back-office/create-case/v4/2-plan-type', {
  ...
  if (isEdit) {
    ...
    res.redirect('/projects/back-office/create-case/v4/3-select-LPA');
  }
});

// Select LPA page
router.get('/projects/back-office/create-case/v4/3-select-LPA', (req, res) => {
  ...
  res.render('projects/back-office/create-case/v4/3-select-LPA', {
    lpaList,
    selectedLPA,
    isEdit,
    index
...
router.post('/projects/back-office/create-case/v4/3-select-LPA', (req, res) => {
  if (!req.body.lpa) {
    const path = require('path');
    const fs = require('fs');
    const lpaListPath = path.join(__dirname, '../data/lpa-list.json');
    ...
    return res.render('projects/back-office/create-case/v4/3-select-LPA', {
      lpaList,
      selectedLPA: '',
      isEdit: req.body.isEdit === 'true',
...
router.get('/projects/back-office/create-case/v4/add-additional-lpa', (req, res) => {
  res.render('projects/back-office/create-case/v4/add-additional-lpa', {
    hasAdditionalLPA: req.session.hasAdditionalLPA
  });
});

router.post('/projects/back-office/create-case/v4/add-additional-lpa', (req, res) => {
  if (!req.body.hasAdditionalLPA) {
    return res.render('projects/back-office/create-case/v4/add-additional-lpa', {
      hasAdditionalLPA: '',
  ...
  if (req.body.hasAdditionalLPA === 'yes') {
    res.redirect('/projects/back-office/create-case/v4/additional-LPA');
  } else {
    res.redirect('/projects/back-office/create-case/v4/main-contact');
  }
});

// Additional LPA page
router.get('/projects/back-office/create-case/v4/additional-LPA', (req, res) => {
  const path = require('path');
  const fs = require('fs');
  const lpaListPath = path.join(__dirname, '../data/lpa-list.json');
  let lpaList = [];
  ...
  res.render('projects/back-office/create-case/v4/additional-LPA', { lpaList });
});

router.post('/projects/back-office/create-case/v4/additional-LPA', (req, res) => {
  if (!req.body.lpa) {
    const path = require('path');
    const fs = require('fs');
    const lpaListPath = path.join(__dirname, '../data/lpa-list.json');
    ...
    return res.render('projects/back-office/create-case/v4/additional-LPA', {
      lpaList,
      selectedLPA: '',
...
router.get('/projects/back-office/create-case/v4/main-contact', (req, res) => {
  const isEdit = req.query.edit === 'true';
  res.render('projects/back-office/create-case/v4/main-contact', {
...
router.post('/projects/back-office/create-case/v4/main-contact', (req, res) => {
  const isEdit = req.body.isEdit === 'true';
  const fromPage = req.body.from || req.query.from || 'check-answers';
  const lpas = req.session.lpas || [];

  if (!req.body.mainContactFirstName || !req.body.mainContactLastName || !req.body.mainContactEmail) {
    return res.render('projects/back-office/create-case/v4/main-contact', {
      lpas,
      mainContact: {
      ...
      isEdit,
      from: fromPage,
      error: 'Enter first name, last name and email address for the contact'
    });
  }
  ...
  if (lpas.length === 1) {
    req.session.mainContact.organisation = lpas[0];
    delete req.session.pendingContactAssociation;
    return res.redirect(returnTo);
  }

  res.redirect('/projects/back-office/create-case/v4/contact-organisation');
});

router.get('/projects/back-office/create-case/v4/change-main-contact', (req, res) => {
  ...
  if (!req.session.mainContact || contacts.length === 0) {
    return res.redirect('/projects/back-office/create-case/v4/check-contact-details');
  }

  res.render('projects/back-office/create-case/v4/change-main-contact', {
...
router.post('/projects/back-office/create-case/v4/change-main-contact', (req, res) => {
  const contacts = req.session.contacts || [];
  ...
  if (!selectedMainContact) {
    return res.render('projects/back-office/create-case/v4/change-main-contact', {
      contactOptions: buildContactOptions(req.session.mainContact, contacts, req.session.lpas || []),
      selectedMainContact: '',
...
router.get('/projects/back-office/create-case/v4/contact-organisation', (req, res) => {
  ...
  const lpas = req.session.lpas || [];
  if (lpas.length === 1) {
    const selectedOrganisation = lpas[0];
    ...
    const returnTo = pending.returnTo || '/projects/back-office/create-case/v4/check-contact-details';
    delete req.session.pendingContactAssociation;
    return res.redirect(returnTo);
  }
  ...
  res.render('projects/back-office/create-case/v4/contact-organisation', {
    lpas,
...
router.post('/projects/back-office/create-case/v4/contact-organisation', (req, res) => {
  const pending = req.session.pendingContactAssociation;
  if (!pending) {
    return res.redirect('/projects/back-office/create-case/v4/check-contact-details');
  }
  ...
  if (!selectedOrganisation) {
    ...
    return res.render('projects/back-office/create-case/v4/contact-organisation', {
      lpas,
      selectedOrganisation,
      contactLabel,
...
// Remove main contact
router.post('/projects/back-office/create-case/v4/remove-main-contact', (req, res) => {
  req.session.mainContact = null;
  res.redirect('/projects/back-office/create-case/v4/main-contact');
});

// Remove additional contact
router.post('/projects/back-office/create-case/v4/remove-contact', (req, res) => {
...
router.post('/projects/back-office/create-case/v4/add-another-contact', (req, res) => {
  if (!req.body.addAnotherContact) {
    return res.render('projects/back-office/create-case/v4/add-another-contact', {
      addAnotherContact: '',
      contacts: req.session.contacts || [],
...
router.get('/projects/back-office/create-case/v4/additional-contact', (req, res) => {
  let contact = {};
  ...
  res.render('projects/back-office/create-case/v4/additional-contact', {
    lpas: req.session.lpas || [],
    contact,
    editIndex,
    fromCheckAnswers,
...
router.post('/projects/back-office/create-case/v4/additional-contact', (req, res) => {
  ...
  if (!req.body.firstName || !req.body.lastName || !req.body.email) {
    return res.render('projects/back-office/create-case/v4/additional-contact', {
      lpas,
      contact: {
      ...
      fromCheckAnswers,
      from: req.body.from || '',
      error: 'Enter first name, last name and email address for the additional contact'
    });
  }
  ...
  if (req.body.editIndex !== undefined && req.body.editIndex !== '') {
    ...
    if (lpas.length === 1) {
      req.session.contacts[editIndex].organisation = lpas[0];
      delete req.session.pendingContactAssociation;
      return res.redirect(returnTo);
    }

    return res.redirect('/projects/back-office/create-case/v4/contact-organisation');
  } else {
    ...
    if (lpas.length === 1) {
      req.session.contacts[newIndex].organisation = lpas[0];
      delete req.session.pendingContactAssociation;
      return res.redirect(returnTo);
    }
...
router.get('/projects/back-office/create-case/v4/remove-contact-details-page', (req, res) => {
  ...
  res.render('projects/back-office/create-case/v4/remove-contact-details', {
    contact,
    editIndex,
    isMain
  });
...
router.post('/projects/back-office/create-case/v4/remove-contact-details-page', (req, res) => {
  ...
  if (isMain) {
    ...
    return res.redirect('/projects/back-office/create-case/v4/main-contact');
  } else if (
    typeof editIndex !== 'undefined' &&
    req.session.contacts &&
    Array.isArray(req.session.contacts) &&
...
router.get('/projects/back-office/create-case/v4/enter-key-dates', (req, res) => {
  const isEdit = req.query.edit === 'true';
  res.render('projects/back-office/create-case/v4/enter-key-dates', {
    mainContact: req.session.mainContact,
    contacts: req.session.contacts || [],
...
router.post('/projects/back-office/create-case/v4/enter-key-dates', (req, res) => {
  const isEdit = req.body.isEdit === 'true';
  ...
  if (errorList.length > 0) {
    return res.render('projects/back-office/create-case/v4/enter-key-dates', {
      mainContact: req.session.mainContact,
      contacts: req.session.contacts || [],
...
// Check answers page
router.get('/projects/back-office/create-case/v4/check-answers', (req, res) => {
  if (!req.session.contacts) req.session.contacts = [];
  if (!req.session.secondaryLPAContacts) req.session.secondaryLPAContacts = [];
  res.render('/projects/back-office/create-case/v4/check-answers', {
    mainContact: req.session.mainContact,
    contacts: req.session.contacts,
...
router.get('/projects/back-office/create-case/v4/clear-data', (req, res) => {
  req.session.contacts = [];
  req.session.mainContact = undefined;
  req.session.lpas = [];
  ...
  req.session.save(() => {
    res.redirect('/projects/back-office/create-case/v4/index');
  });
});

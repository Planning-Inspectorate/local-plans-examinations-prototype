...
function setGw2V4StatusState(req, state) {
  ...
  const clearKeys = [
    WORKSHOP_DOCS_KEY,
    ISSUE_REPORT_DOCS_KEY,
    'gw2v4ReportIssued',
    'gw2v4StatusOverride',
    'gw2v4ProceduralDocuments',
    'gw2v4ConsultationDocuments',
    'gw2v4AdditionalDocuments',
    'gateway2ActualDate',
    'gateway2ValidDate',
    'gateway2WorkshopVenue',
    'gateway2WorkshopDate',
    'gateway2AssessorName',
    'gateway2AssessorAppointmentDate',
    'gateway2ReportIssuedDate',
    'gateway2ReportPublishedDate',
    'hearings',
    'hearingStartDate',
    'hearingTime',
    'hearingEndTime',
    'hearingEstimatedDays',
    'hearingActualDuration',
    'hearingEndDate',
    'hearingIsVirtual',
    'hearingHasVirtualMeetingLink',
    'hearingVirtualMeetingLink',
    'hearingVenue',
    'hearingAddress',
    'hearingHasAddress'
  ];

  clearKeys.forEach((key) => delete req.session[key]);
  if (req.session.data) {
    clearKeys.forEach((key) => delete req.session.data[key]);
  }
  ...
  if (state === 'workshop-confirmed') {
    req.session.gw2v4StatusOverride = 'GW2 workshop confirmed';
    req.session.gateway2EstimatedDate = req.session.gateway2EstimatedDate || defaultEstimatedDate;
    req.session.gateway2WorkshopVenue = defaultWorkshopVenue;
    req.session.gateway2WorkshopDate = defaultWorkshopDate;
    req.session.data.gw2v4ProceduralDocuments = req.session.gw2v4ProceduralDocuments;
...
function getGateway2Status(req, workshopDocuments) {
  ...
  if (req.session.gw2v4StatusOverride === 'GW2 workshop confirmed') {
    return {
      text: 'GW2 workshop confirmed',

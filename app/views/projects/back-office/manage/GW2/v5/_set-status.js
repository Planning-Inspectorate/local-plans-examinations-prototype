...
function clearGateway2ReportIssuedState(req) {
  delete req.session.gw2v5IssueReportDocuments;
  delete req.session.gateway2ReportIssuedDate;
  req.session.gw2v5ReportIssued = false;

  if (req.session.data) {
    delete req.session.data.gw2v5IssueReportDocuments;
  }
}
...
router.get('/set-status', (req, res) => {
  ...
  req.session.gw2v5StatusState = state;
  ...
  if (state === 'gw3-submission') {
    ...
    if (!req.session.gw2v5IssueReportDocuments || req.session.gw2v5IssueReportDocuments.length === 0) {
      req.session.gw2v5IssueReportDocuments = [
        {
    ...
    if (!req.session.gw2v5ReportDocuments || req.session.gw2v5ReportDocuments.length === 0) {
      req.session.gw2v5ReportDocuments = [
        {
